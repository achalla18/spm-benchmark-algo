"""
Scale-free query size sweep — Montevideo region.
...
all_results = {}
for q in Q_SIZES:
    pat = make_scale_free_pattern(q)
    res, meta = run_one(N_DB, pat, label=f"SF Q={q}")
    all_results[q] = (res, meta)

now   = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
lines = [
    "# Montevideo — Scale-Free Query Size Sweep\n",
    f"_Run date: {now}_\n",
    f"**Region:** Montevideo, Uruguay — lat [{LAT_MIN}, {LAT_MAX}] x lon [{LON_MIN}, {LON_MAX}]  (1 deg x 1 deg)\n",
    "",
    "| Q | BA edges | MPJ matches | MPJ time | MSJ matches | MSJ time | ESPM matches | ESPM time |",
    "|---|----------|-------------|----------|-------------|----------|--------------|-----------|",
]
