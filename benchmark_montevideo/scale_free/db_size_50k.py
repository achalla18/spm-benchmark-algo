"""
Scale-free DB size sweep — Montevideo region.
Q=20 fixed (BA m=2).  DB = 1,000 / 10,000 / 50,000 nodes.
"""
import os, sys
from datetime import datetime, timezone

sys.path.insert(0, os.path.normpath(os.path.join(os.path.dirname(__file__), '..')))
from common import make_scale_free_pattern, run_one, fmt_result, LAT_MIN, LAT_MAX, LON_MIN, LON_MAX
...
for n_db in DB_SIZES:
    res, meta = run_one(n_db, pattern, label=f"SF DB={n_db}")
    all_results[n_db] = (res, meta)

now   = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
lines = [
    "# Montevideo — Scale-Free Query, DB Size Sweep\n",
    f"_Run date: {now}_\n",
    f"**Region:** Montevideo, Uruguay — lat [{LAT_MIN}, {LAT_MAX}] x lon [{LON_MIN}, {LON_MAX}]  (1 deg x 1 deg)\n",
    f"**Query:** {N_QUERY} nodes, BA scale-free m=2, {len(pattern)} edges\n",
    "",
