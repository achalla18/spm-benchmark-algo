"""
Mesh DB size sweep — Montevideo region.
Q=20 fixed (ring-lattice k=4, 40 edges).  DB = 1,000 / 10,000 / 50,000 nodes.
"""
import os, sys
...
for n_db in DB_SIZES:
    res, meta = run_one(n_db, pattern, label=f"Mesh DB={n_db}")
    all_results[n_db] = (res, meta)

now   = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
lines = [
    "# Montevideo — Mesh Query, DB Size Sweep\n",
    f"_Run date: {now}_\n",
    f"**Region:** Montevideo, Uruguay — lat [{LAT_MIN}, {LAT_MAX}] x lon [{LON_MIN}, {LON_MAX}]  (1 deg x 1 deg)\n",
    f"**Query:** {N_QUERY} nodes, ring-lattice k=4, {len(pattern)} edges\n",
    "",
