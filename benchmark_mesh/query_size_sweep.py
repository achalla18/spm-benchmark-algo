"""
query_size_sweep.py  (mesh / ring-lattice query)
=================================================
DB=1,000 fixed.  Query size varied: 20, 40, 60 nodes.
Query graph: ring-lattice, degree k=4.

Results -> benchmark_mesh/results/query_size_sweep.md
"""
...
def write_results(all_results):
    ...
    lines = [
        ...
        f"_Run date: {now}_\n",
        f"Query graph: ring-lattice (degree k={MESH_K}). "
    ...
    for n_q in QUERY_SIZES:
        mesh = len(make_mesh_pattern(n_q))
        fc   = n_q * (n_q - 1) // 2
        sf   = {"20": 37, "40": 77, "60": 117}.get(str(n_q), "?")
        lines.append(f"| {n_q} | {mesh} | {sf} | {fc} |")
