"""
db_size_50k.py  (mesh / ring-lattice query)
============================================
Q=20 fixed (ring-lattice, k=4, 40 edges).
DB size varied: 1,000 / 10,000 / 50,000 nodes.

Results -> benchmark_mesh/results/db_size_50k.md
"""
...
from common import run_one, make_mesh_pattern, TIMEOUT_S, MAX_MATCHES, MESH_K
...
def write_results(all_results):
    ...
    n_edges = len(make_mesh_pattern(N_QUERY))
    ...
    lines = [
        ...
        f"_Run date: {now}_\n",
        f"Query: {N_QUERY} nodes, ring-lattice k={MESH_K}, {n_edges} edges. "
    ...
    for algo in ["MPJ", "MSJ", "ESPM"]:
        ...
        def _c(t,to): return "timeout" if to else f"{t:.4f}"
        def _f(i,j):
            t1,to1=times[i]; t2,to2=times[j]
            return "N/A" if (to1 or to2) else f"{t2/t1:.1f}x"
        t1,to1=times[0]; t2,to2=times[1]; t3,to3=times[2]
        lines.append(
            f"| {algo} | {_c(t1,to1)} | {_c(t2,to2)} | {_c(t3,to3)} "
            f"| {_f(0,1)} | {_f(1,2)} |"
        )

    lines += ["", "---", "", "## Notes", "",
              f"- Query: {N_QUERY}-node ring-lattice, k={MESH_K}, {n_edges} edges.",
